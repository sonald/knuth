...
>
> 注：`MessageMiddleware` 的最新定义见
> [MessageMiddleware 需求与设计](message-middleware-requirements-and-design.md)。它已从
> 一次性的 `ContextView` mutator 修订为生命周期 checkpoint 上的 `MessageTape` rewrite
> 机制。
...
> **⚠️ 已被 [ADR-008](decisions/ADR-008-tool-owned-observation-condensation.md) / [ADR-009](decisions/ADR-009-standalone-filesystem-artifact-store.md) 取代（superseded）。** 本节描述 v0 旧设计：artifact 为 ledger blob 侧店、事件持 `artifact_ref` + `observation_preview`、超阈值复水进 tape。新方案把 artifact 抽成**独立 FS 存储**、工具**自我压缩**、`observation` **必填**、删除 `observation_preview` 与复水。下文留作历史记录。

事件 payload 只放最小事实；超过阈值（v0 定 8KB）的内容（工具结果、文件内容、
原始响应）写入 ArtifactStore，事件持有 `artifact_ref`，hash 由 ArtifactStore 记录。
...
- 事件类型一经发布（首个 tagged release 起），**形状冻结**。演进 = 追加新类型
  （或 `.v2` 后缀类型），不改旧类型。
- 所有 fold 必须容忍未知事件类型（跳过）；事件模型不开放任意额外字段，避免
  schema 债从 `dict` 逃生口重新进入核心模型。
...
| `model.completed` | `step_id, content, tool_calls: [{tool_call_id, index, name, args}], finish_reason, usage` | 本轮模型产出的最小事实；不嵌整条 InferenceMessage |
...
| `approval.requested` | `approval_id, tool_call_id, args_hash, title, reason, risk, approval_preview`（已脱敏） | |
| `approval.resolved` | `approval_id, resolution: approved \| denied, resolved_by?` | 不直接改 run 状态；resume 由 `run.resumed` 表达 |
| `tool.invocation_started` | `tool_call_id, attempt` | |
| `tool.invocation_completed` | `tool_call_id, outcome: succeeded \| failed \| denied, observation \| artifact_ref, observation_preview, tool_status?` | observation 是模型将看到的文本（≤8KB 内联，否则 artifact ref）。**artifact 部分（`artifact_ref` / `observation_preview` / 复水）已被 ADR-008/009 取代** |
...
| `message.rewrite_anchor` / `message.rewrite_message` | `rewrite_id, operation, position/suppresses, replacement message(s), metadata` | 通用 MessageTape rewrite 事实；`context_compaction` 是 middleware 名称，不是单独事件协议 |
...
message.rewrite_*           -> internal anchors + replacement messages，按 target span / position 投影
```
...
create table tool_invocations (
  ...
  observation_recorded integer not null,
...
v0 不承诺通用幂等与回滚体系。对可能产生外部副作用的工具，崩溃恢复只会进入
UNKNOWN 并等待人工裁决；未来如果引入幂等能力，需要为具体 provider 定义稳定
key 的作用域、生成规则和消费契约。
...
```text
1. prepare      AgentLoop / RunInvocation 在安全 checkpoint 运行 durable middleware
2. assemble     ConversationProjection fold + durable rewrite events + SystemSectionProvider
3. project      应用 MessageTape anchors / suppressions / ephemeral insertions
4. tool_filter  按 policy 过滤可见工具
5. freeze       生成 ContextSnapshot，此后视图不可再修改
```

顺序固定解决"读路径偷偷写 ledger"、"replacement 跑到对话尾部"、"最终消息序列不合法"这类次序事故。redaction / compaction 不再是 `ContextView` 上的任意 mutation，而是由 durable rewrite events 和 projection 规则表达。

### 6.2 扩展面（沿用 CONTEXT.md 已定的收权决定）

- `SystemSectionProvider`：稳定 preamble fragment 的常规入口，只增不改。
- `MessageMiddleware`：绑定生命周期 checkpoint 的 `MessageTape` rewrite 组件，用
  durable anchors 和 replacement messages 表达 AgentsMD 注入、tool result redaction 和
  context compaction。
- middleware 输出结构化 patch，并由 runner / ledger 写成可审计、可恢复的 rewrite events。
...
2. secret 明文不进 ledger、不进 ArtifactStore；工具通过 secret handle 取用，
   模型与事件只见 handle 描述。
3. `approval.requested.approval_preview` 必须经过脱敏。
...
`ToolRuntimeContext` 分两半：数据（run_id、tool_call_id，
Pydantic）+ 能力句柄（put_artifact、emit_progress、
...
| crash 恢复未定义 | effect 分流：可重试 / UNKNOWN 人工裁决 |
...
4. **崩溃恢复**：effect 分流、UNKNOWN、`knuth resolve`。
5. **verification feedback**。
6. **安全红线**：redact-before-append、approval preview 脱敏、debug sink、entry points
   默认关。
7. **工具模型拆分 + 超时/取消**。
8. **context pipeline 阶段化 + ContextSnapshot**。

预留不实现（事件类型留位即可）：`run.checkpoint`（fold 成本成为瓶颈时）、
daemon / worker lease、Workflow IR、tool cache、
