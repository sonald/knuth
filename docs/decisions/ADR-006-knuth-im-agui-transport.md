...
推论:没有 Node 中间层来补协议或纠漂移,因此 `knuth-agui` 必须自己把 AG-UI 实现完整,并尽早改用官方 `ag-ui` Python encoder（而非 spike 里手搓的事件构造）。前端声明的工具(`RunAgentInput.tools`)也必须由 `knuth-agui` 自己处理。
...
**组合点(关键)**:`knuth-agui` 唯一的入口是 `create_app(runtime: AgentRuntime)`——它**只接收一个已经构造好的 `AgentRuntime`**,自己绝不构造 runtime、绝不决定 prompt / tool provider / policy。"knuth-im 复用 knuth-cli 的工具集与系统提示"(决策 1)这件事发生在 **knuth-im 的 host / runtime factory** 层(与 knuth-cli 各自构造自己的 runtime 是对等关系),不在 transport 包里。换言之:谁决定"用哪些工具、什么系统提示、什么 policy"是 agent 策略,归 host;`knuth-agui` 只把构造好的 runtime 翻译上线。这条把决策 1(复用 CLI 工具)与"transport 不持有策略"两句话钉在一起,防止 M3 接 `create_cli_tool_provider` 时把策略塞进 transport 包。

> 注:M1 spike 里 `knuth-agui/runtime_factory.py` 的 `build_spike_runtime` 违反了这一点(把 runtime 构造放进了 transport 包)——这是 spike 便利,M2/M3 必须把 runtime factory 迁出 `knuth-agui`,落到 knuth-im host(`apps/knuth-im-web` 的后端入口或独立 `knuth-im` host 模块),`knuth-agui` 退回纯 `create_app(runtime)` 库。
...
- **invocation 级工具 overlay**(不是简单的 per-run `tool_providers` 注入):前端声明的工具只对**本次 invocation**可见,绝不能进 runtime 级共享 registry。当前 `ToolRegistry` 是 runtime 级共享的 provider/index,`add_provider()` 写全局 `_providers` 且命名冲突直接 raise、`refresh()` 按全局 provider 重建索引([registry.py](../../packages/knuth-toold/src/knuth_toold/registry.py));每个浏览器 run 直接 `add_provider` 会造成跨 run 泄漏、命名冲突、并发污染。因此 seam 是 `ToolBroker` 的 **invocation-scoped overlay/view**:在共享 registry 之上叠一层只在本次 run 生命周期内有效的工具集,与共享索引隔离。
...
v1 同时包含服务端 CLI 工具(read/write/shell/search/glob)与前端工具(`useCopilotAction`)及其生成式 UI 渲染。这把最难的 seam(invocation 级工具 overlay + 客户端工具回传)纳入 v1,因为"带生成式能力"是 knuth-im 的核心卖点,且直连拓扑(决策 2)要求 `knuth-agui` 本就要处理 `tools` 数组。

### 6. 客户端工具 seam:方案 A(Knuth 原生)✅

模型调用前端声明的工具时,run 必须挂起等浏览器执行完再回传。两种建模待实测后定:

- **方案 A(Knuth 原生,已选)**:前端工具经 invocation 级工具 overlay(决策 3)进入本次 run;agent loop 遇到这类工具不本地执行,而是把该 `ToolInvocation` 置入一个**新的中性等待态**等浏览器结果;前端 `POST /tool_result` → 追加 `ToolInvocationCompleted` → 再打开新的 `/agent` resume 流。客户端工具仍是一等 `ToolInvocation`,走完 batch/审计机制。
...
- 新增包 `knuth-agui` 收敛为纯 transport 库(只 `create_app(runtime)`);runtime factory 归 knuth-im host(M2/M3 把 spike 的 `build_spike_runtime` 迁出 transport 包)。新增前端应用 `apps/knuth-im-web`,仓库自此混 Python(uv workspace)与 JS 两套工具链。
...
- runtime 将获得几个加性 seam(`start(prompt, run_id=None)` 透传到 `ledger.create_run`、invocation 级工具 overlay、客户端工具的新中性等待态 + 补录),每个都与受众无关,可被 CLI/daemon 同样使用;runtime 不出现任何 HTTP/AG-UI 字样。
...
- 客户端工具采用方案 A 后,M4 的核心协议 seam 已落入 runtime/transport/frontend 三层;更丰富的生成式组件可在不改协议的前提下继续扩展。
...
- **M3** ✅ 服务端工具 + 审批:接 `create_cli_tool_provider` + policy、CUSTOM 审批 + `/approve`(仅落库解决)+ 前端开 resume 流、富渲染。
- **M4** ✅ 前端工具协议:方案 A 已定并实现 invocation 级工具 overlay + 客户端工具新等待态 + `/tool_result` + 前端自动 resume。生成式组件继续作为 UI 增强迭代。
